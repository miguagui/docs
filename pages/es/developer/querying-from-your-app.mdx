...
Otra posible opción es obtener los datos en Python, para ello, bastará con utilizar el siguiente código :
...
url = 'https://api.thegraph.com/subgraphs/name/username/subgraphname'
consult = requests.post(url, json={'query': query})
json_data = json.loads(consult.text)
df_data = json_data['data']['tokens']
df = pd.DataFrame(df_data)
```
Con ello obtendrás un dataframe con las columnas id, tokenID, contentURI
